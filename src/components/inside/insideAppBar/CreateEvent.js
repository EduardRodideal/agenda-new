...
export const CreateEvent = () => {
  ...
  const url =
    proxyurl +
    "https://europe-west3-agenda-idealante.cloudfunctions.net/api/item";

  //sends the item details to the datebase
  const handleClickSave = (event) => {
    event.preventDefault();
    axios
      .post(url, itemDetails, config)
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
  ...
  return (
    <div>
      {/**the cross icon that is on the Navbar */}
      <AddCircleIcon fontSize="large" className="plus" onClick={handleOpen} />
      <Modal
